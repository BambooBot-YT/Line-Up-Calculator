...
/// </summary>
public class MollyControllerSpeed : MonoBehaviour
{
    // Private fields
    /// <summary>
    ...
    private HashSet<Vector3> validPaths = new();

    private int currentIndex = 0;
    private bool isSimulating = false;
    private readonly int MAX_MILLIS_PER_FRAME = 16;
    ...
    private void Update()
    {
        if (Input.GetKeyDown(simulationKey) || isSimulating)
        {
            if (!isSimulating) currentIndex = 0;
            if (!isSimulating)
            {
                validPaths.Clear();
    ...
    private IEnumerator CalculatePath(Vector3 vector)
    {
        Vector3 position = origin.position;
        Vector3 velocity = vector.normalized * molly.throwForce;
        float time = 0;
        ...
        while (step++ <= maxStepCount)
        {
            ...
            if (Physics.Raycast(position, velocity, out RaycastHit hit, velocity.magnitude * stepSize, 3))
            {
                ...
                if (bounces++ == molly.targetBounces)
                {
                    if (Vector3.Distance(position, target.position) < molly.targetRadius)
                    {
                        LineupCamera.addLineup(position, vector);
    ...
    /// <summary>
    /// Begins the solver process for all vectors.
    /// </summary>
    public void BeginSolver()
    {
        System.Diagnostics.Stopwatch watch = new System.Diagnostics.Stopwatch();
        watch.Start();
        for (int i = currentIndex; i < vectors.Length; i++)
        {
            StartCoroutine(CalculatePath(vectors[i]));
            if (watch.ElapsedMilliseconds >= MAX_MILLIS_PER_FRAME)
            {
                isSimulating = true;
                currentIndex = i + 1;
                watch.Stop();
                Debug.Log($"Simulation is {currentIndex / (float)vectors.Length * 100}% complete.");
                return;
            }
        }
        isSimulating = false;
